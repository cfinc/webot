...
@Component
@Slf4j
public class EmojiCallbackMessageHandler extends AbstractCallbackMessageHandler<NormalCallBackMessage>{
    @Override
    public boolean support(NormalCallBackMessage normalCallBackMessage) {
        ...
        try {
            ...
            return normalCallBackMessage.typeSupport(47);
        } catch (Exception e) {
    ...
    public void handleMessage(NormalCallBackMessage normalCallBackMessage) {
        log.info("表情消息验证通过，开始处理文本消息 - {}", JSON.toJSONString(normalCallBackMessage));

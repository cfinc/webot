...
@Component
@Slf4j
public class FileSentCompleteCallbackMessageHandler extends AbstractCallbackMessageHandler<NormalCallBackMessage> {
    @Override
    ...
    public void handleMessage(NormalCallBackMessage normalCallBackMessage) {
        log.info("文件发送完毕消息验证通过，开始处理文本消息 - {}", JSON.toJSONString(normalCallBackMessage));

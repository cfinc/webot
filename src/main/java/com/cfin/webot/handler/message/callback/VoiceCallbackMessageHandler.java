...
@Component
@Slf4j
public class VoiceCallbackMessageHandler extends AbstractCallbackMessageHandler<NormalCallBackMessage>{
    @Override
    public boolean support(NormalCallBackMessage normalCallBackMessage) {
        ...
        try {
            ...
            return normalCallBackMessage.typeSupport(34);
        } catch (Exception e) {
    ...
    public void handleMessage(NormalCallBackMessage callBackMessage) {
        log.info("语音消息验证通过，开始处理文本消息 - {}", JSON.toJSONString(callBackMessage));

...
@Component
@Slf4j
public class AddFriendCallbackMessageHandler extends AbstractCallbackMessageHandler<NormalCallBackMessage>{
    @Override
    public boolean support(NormalCallBackMessage normalCallBackMessage) {
        ...
        try {
            ...
            return normalCallBackMessage.typeSupport(37);
        } catch (Exception e) {
    ...
    public void handleMessage(NormalCallBackMessage normalCallBackMessage) {
        log.info("添加好友请求消息验证通过，开始处理文本消息 - {}", JSON.toJSONString(normalCallBackMessage));
